...
pub fn main() {
    ...
    std::panic::set_hook(Box::new(|info| {
        ...
        let err_msg = format!(
            "IF mod plugin has panicked at '{}' with the following message:\n{}\0",
            location, msg
        );
        ...
        skyline::error::show_error(
            69,
            "IF mod plugin has panicked! Please open the details and send a screenshot to the developer, then close the game.\n\0",
            err_msg.as_str(),
        );
    }));
